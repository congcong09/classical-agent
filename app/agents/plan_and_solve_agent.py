...
你是一个顶级的AI规划专家。你的人物是将用户提出的复杂问题分解成一个由多个简单步骤组成的计划。
...
你是一位顶级的AI执行专家，你的任务是严格按照给定的几乎，一步步的解决问题。
你将收到原始问题、完整计划，以及目前为止已经完成的步骤和结果。
...
class Executor:
    ...
    def execute(self, question, plan: list[str]):
        """
        根据计划，逐步执行并解决问题
        """
        history = ""
